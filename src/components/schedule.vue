      ...
      <el-row>
        <el-collapse v-model="activeName" accordion>
          <el-collapse-item :title="lastchecktime" name="1">
            <el-row :gutter="5">
              <el-select @change="updatelevel2option" clearable v-model="task_level1_select" style="width: 120px"
                placeholder="请选择">
              ...
              <el-select v-model="task_level3_select" filterable clearable allow-create style="width: 120px"
                placeholder="请选择">
                <el-option v-for="item in task_level3_option" :key="item.value" :label="item.label"
                  :value="item.value"></el-option>
              </el-select>
              <el-select v-model="schedule_type" filterable clearable allow-create style="width: 120px"
                placeholder="请选择">
                <el-option v-for="item in schedule_type_option" :key="item.value" :label="item.label"
                  :value="item.value"></el-option>
              </el-select>
        ...
        <el-table :data="scheduledata" border height="750" style="width: 100%" @cell-click="showscheduleprocess">
          ...
          <el-table-column label="操作" width="120">
            <template slot-scope="scope">
              ...
              <el-button v-if="scope.row.isabandon === '启用'" @click="forbidschedule(scope.row)" type="text"
                size="small">禁用</el-button>
              <el-button v-if="scope.row.isabandon === '禁用'" @click="startschedule(scope.row)" type="text"
    ...
    getscheduledata: function() {
      axios.get('/getscheduledata').then((response) => {
        if (response.status === 200) {
          ...
          for (const i in this.scheduledata) {
