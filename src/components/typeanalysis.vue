<template>
  <div id="app">
    <el-row>
      <el-col :span="3">
        <el-select @change="updatesuboption" clearable filterable allow-create default-first-option
          v-model="task_select" placeholder="请选择">
          <el-option v-for="item in task_select_option" :key="item.value" :label="item.label"
            :value="item.value"></el-option>
        </el-select>
      </el-col>
      <el-col :span="4">
        <el-select v-model="task_sub_select" filterable clearable allow-create default-first-option placeholder="请选择">
          <el-option v-for="item in task_sub_select_option" :key="item.value" :label="item.label"
            :value="item.value"></el-option>
        </el-select>
      </el-col>
      <el-col :span="4">
        <el-select v-model="person_id" filterable :filter-method="personFilter" clearable default-first-option
          placeholder="请选择相关人员">
          <el-option v-for="item in person_option" :key="item.value" :label="item.label"
            :value="item.value"></el-option>
        </el-select>
      </el-col>
      <el-col :span="3"><el-button type="primary" @click="freshscatter">查询</el-button></el-col>
    </el-row>
    <el-row>
      <el-col :span="20">
  ...
  methods: {
    freshscatter: function() {
      axios
        ...
        .then((response) => {
          const temp_data = response.data.scatter_data;
    ...
    updatesuboption: function(event) {
      ...
      if (this.task_select !== '' && this.task_sub_all_option && this.task_sub_all_option[this.task_select]) {
        this.task_sub_select_option = [];
        const temp = this.task_sub_all_option[this.task_select];
        // console.log(temp);
        if (temp) {
          this.task_sub_select_option = [];
          for (const i in temp) {
