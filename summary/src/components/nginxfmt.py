        ...
        if line == "":
            ...
            if line.startswith("#"):
                ...
                if q == 1 and c > 1:
                    ...
                    newlines = line.split(";")
                    cleaned_lines.extend(clean_lines(
                        ["".join([ln, ";"]) for ln in newlines if ln != ""]))
                else:
                    if line.startswith("rewrite"):
                        cleaned_lines.append(
                            strip_variable_template_tags(line))
                    else:
...
def format_config_file(file_path, original_backup_file_path=None, verbose=True):
    ...
    if verbose:
        print("Formatted file '%s' (detected encoding %s)." %
              (file_path, chosen_encoding))
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument("-b", "--backup-original",
                            action="store_true", help="backup original config file")
