<template>
  ...
  <div id="app">
    ...
    <el-dialog @close="dialogtransactionVisible = false" title="修改" :visible="dialogtransactionVisible" width="40%">
      <div>
        <el-row :span="6">
          <el-select @change="updatelevel2option" v-model="level1" filterable allow-create default-first-option
            placeholder="请选择一级" style="width: 140px">
            <el-option v-for="item in level1_option" :key="item.value" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
          <el-select @change="updatelevel3option" v-model="level2" filterable allow-create default-first-option
            placeholder="请选择二级" style="width: 140px">
            <el-option v-for="item in level2_option" :key="item.value" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
          <el-select v-model="level3" filterable allow-create default-first-option placeholder="请选择三级"
            style="width: 140px">
            <el-option v-for="item in level3_option" :key="item.value" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
          <el-select v-model="d_data_status" clearable default-first-option style="width: 120px" placeholder="请选择状态">
        ...
        <el-row :span="6"><el-input v-model="d_product" style="width: 300px" :disabled="true"></el-input></el-row>
      </div>
...
export default {
  ...
  methods: {
    ...
    updatelevel2option: function(event) {
      ...
      this.level2 = '';
      if (this.level1 !== '' && this.level1_level2_option && this.level1_level2_option[this.level1]) {
        this.level2_option = [];
        const temp = this.level1_level2_option[this.level1];
        if (temp) {
          this.level2_option = [];
          for (const i in temp) {
    ...
    updatelevel3option: function(event) {
      // console.log('update option');
      this.post = '';
      if (this.level2 !== '' && this.level2_level3_option && this.level2_level3_option[this.level2]) {
        this.level3_option = [];
        const temp = this.level2_level3_option[this.level2];
        if (temp) {
          this.level3_option = [];
          for (const i in temp) {
