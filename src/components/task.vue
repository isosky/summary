  ...
  <div id="app">
    <el-row :gutter="5">
      ...
      <el-col :span="11">
        ...
        <el-row :gutter="5">
          ...
          <el-col :span="6">
            <el-date-picker :picker-options="{ firstDayOfWeek: 1 }" v-model="new_edate" value-format="yyyy-MM-dd"
              type="date" placeholder="ddl" @change="setdate"></el-date-picker>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="4">
        ...
        <el-row :gutter="5">
          <el-col :span="10">
            <el-date-picker v-model="query_duration" type="daterange" unlink-panels value-format="yyyy-MM-dd"
              ...
              <el-table :data="tableprocess" border height="455" style="width: 100%">
                ...
                <el-table-column label="操作" width="170">
                  <template slot-scope="scope">
                    ...
                    <el-button v-if="scope.row.isfinish === '完成'" @click="resetprocess(scope.row)" type="text"
                      size="small">待做</el-button>
                    <el-button v-if="scope.row.isfinish === '待做'" @click="finishprocess(scope.row)" type="text"
          ...
          <el-form ref="form" :model="finishtaskform" label-width="80px">
            ...
            <el-row>
              <el-col :span="12">
                <el-row>
                  <el-col :span="6">
                    <el-select @change="updatediroption" clearable filterable allow-create default-first-option
                      v-model="dir_select" placeholder="请选择">
                      <el-option v-for="item in dir_select_option" :key="item.value" :label="item.label"
                        :value="item.value"></el-option>
                    </el-select>
                  </el-col>
                  <el-col :span="6">
                    <el-select v-model="dir_sub_select" filterable clearable allow-create default-first-option
                      placeholder="请选择">
                      <el-option v-for="item in dir_sub_select_option" :key="item.value" :label="item.label"
                        :value="item.value"></el-option>
                    </el-select>
                  </el-col>
                  <el-col :span="6">
              ...
              <el-col :span="12">
                ...
                <el-row>
                  <el-table ref="singleTable" :data="finishtaskform.peoples" style="width: 100%" class="tb-edit"
                    ...
                    <el-table-column prop="score_activity" label="积极性" align="center">
                      <template scope="scope">
                        <el-input size="small" v-model="scope.row.score_activity" placeholder="请输入内容"
                    ...
                    <el-table-column prop="score_critical" label="批判性" align="center">
                      <template scope="scope">
                        <el-input size="small" v-model="scope.row.score_critical" placeholder="请输入内容"
                          @change="handleEdit(scope.$index, scope.row)"></el-input>
    ...
    <el-dialog @close="closedialog" title="确认修改任务？？" :visible.sync="dialoguVisible" width="40%">
      <el-row>
        ...
        <el-date-picker :picker-options="{ firstDayOfWeek: 1 }" v-model="duetime" value-format="yyyy-MM-dd" type="date"
          style="width: 150px"></el-date-picker>
        <span class="demonstration" style="margin-left: 15px;">实际结束时间：</span>
        <el-date-picker :picker-options="{ firstDayOfWeek: 1 }" v-model="dftime" value-format="yyyy-MM-dd HH:mm:ss"
          type="datetime" style="width: 250px"></el-date-picker>
        <el-select v-model="dustatus" placeholder="请选择">
          <el-option v-for="item in dustatusoptions" :key="item.value" :label="item.label" :value="item.value">
          </el-option>
        </el-select>
      </el-row>
      <el-row>
...
export default {
  ...
  mounted: function() {
    ...
    this.tasksummary_chart.on('click', function(params) {
      ...
      const temp_sub_select = temp.split('-');
  ...
  methods: {
    ...
    updatelevel2option: function(event) {
      ...
      if (this.task_level1_select !== '' && this.task_sub_all_option && this.task_sub_all_option[this.task_level1_select]) {
        ...
        const temp = this.task_sub_all_option[this.task_level1_select];
        // console.log(temp);
        if (temp) {
          this.task_level2_option = [];
          for (const i in temp) {
    ...
    updatelevel3option: function(event) {
      ...
      if (this.task_level2_select !== '' && this.level2_level3 && this.level2_level3[this.task_level2_select]) {
        ...
        const temp = this.level2_level3[this.task_level2_select];
        // console.log(temp);
        if (temp) {
          this.task_level3_option = [];
          for (const i in temp) {
    ...
    updatediroption: function(event) {
      ...
      if (this.dir_select !== '' && this.dir_sub_all_option && this.dir_sub_all_option[this.dir_select]) {
        ...
        const temp = this.dir_sub_all_option[this.dir_select];
        // console.log(temp);
        if (temp) {
          this.dir_sub_select_option = [];
          for (const i in temp) {
    ...
    getprocess: function(task_id) {
      axios
        ...
        .then((response) => {
          ...
          for (const i in this.tableprocess) {
